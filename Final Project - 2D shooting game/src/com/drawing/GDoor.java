...
public class GDoor implements GShape {
	...
	int currentAnimation = CLOSE;
	
	// constructor to initialize the sprite
	GDoor(final GL2 gl, int x, int y, long interval, float w, float h) {
		...
		for (int i = 0; i < TOTAL_ANIM; i++) {
			...
			for (int j = 0; j < frameNo[i]; j++) {
				...
				String name = "/world/door/" + fileName[i] + "/" + fileName[i] + (j + 1) + ".png";
