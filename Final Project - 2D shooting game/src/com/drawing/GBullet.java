...
public class GBullet implements GShape {
	...
	int TOTAL_FRAME = 9; // we calculate this for each animation
	...
	int currentAnimation = BULLET;
	
	boolean startFiring = false;
	boolean flyDirection = true;

	// constructor to initialize the sprite
	GBullet(final GL2 gl, int x, int y, long interval, float w, float h) {
	...
	// This method will set the of bullet objects, and it will keep the same direction as character
	public void setPosition(int x, int y, boolean direct) {
		this.movX = x;
		this.movY = y;
		this.flyDirection = direct;
	}
	
	//This method will change the fire status which is one important component before firing.
	public void setFiringStatus(boolean e){
		startFiring = e;
	}
	
	//The getter method of position x
	public float getXPostion(){
		return this.movX;
	}
	
	//The getter method of position y
